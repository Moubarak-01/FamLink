...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Listen for incoming messages globally to update state
  ...
  const handleNotificationClick = async (notification: Notification) => {
      ...
      } catch (e) { console.warn("Failed to mark read"); }
  ...
  const handleClearNotifications = async () => {
      if (currentUser) {
          try {
              ...
              setNotifications([]);
          } catch(e) { console.warn("Failed to clear notifications"); }
      }
  };
  ...
  const handleLogin = async (email: string, password: string, rememberMe: boolean) => {
    try {
         ...
         if (data.user.userType === 'parent') {
            ...
            if (data.user.profile) navigateTo(Screen.Dashboard);
            else if (data.user.assessmentResult?.decision === 'Approved') navigateTo(Screen.NannyProfileForm);
            else if (data.user.assessmentResult) navigateTo(Screen.Result);
            else navigateTo(Screen.Questionnaire);
         }
    } catch (err: any) {
  ...
  const handleForgotPassword = async (email: string) => {
    ...
    alert(t('alert_forgot_password_sent'));
  ...
  const handleSubscribe = async (plan: Plan) => {
    ...
    try {
        const updatedUser = await userService.updateProfile({ subscription: newSubscription });
        setCurrentUser(updatedUser);
        alert(t('alert_subscription_success'));
        
        if (pendingAction) {
            const action = { ...pendingAction };
            setPendingAction(null);
            if (action.type === 'contact') setContactNannyInfo(action.nanny);
            if (action.type === 'book') setBookingNannyInfo(action.nanny);
        }
        navigateTo(Screen.Dashboard);
    } catch(e) {
  ...
  const handleNannyProfileSubmit = async (profileData: any) => {
    if(!currentUser) return;
    try {
  ...
  const handleParentProfileSubmit = async (profileData: any) => {
    if (!currentUser) return;
    try {
  ...
  const handleEditProfile = () => {
      ...
      if (currentUser.userType === 'parent') {
          navigateTo(Screen.ParentProfileForm);
      } else {
          navigateTo(Screen.NannyProfileForm);
      }
  };

  const handleViewSubscription = () => {
      navigateTo(Screen.SubscriptionStatus);
  };

  const handleCancelSubscription = async () => {
    if (currentUser?.subscription) {
      const updatedSubscription = { ...currentUser.subscription, status: 'canceled' as const };
      try {
          const updatedUser = await userService.updateProfile({ subscription: updatedSubscription });
          setCurrentUser(updatedUser);
      } catch (e) { alert("Error canceling subscription"); }
    }
  };
  
  const handleContinueFromResult = () => {
      if (currentUser?.userType === 'nanny' && currentUser.assessmentResult?.decision === 'Approved') {
          navigateTo(Screen.NannyProfileForm);
      }
  };

  const handleViewNannyProfile = (nannyId: string) => {
    setViewingNannyId(nannyId);
    navigateTo(Screen.NannyProfileDetail);
  };
  
  const handleContactAttempt = (nanny: User) => {
     if (!currentUser) {
        setPendingAction({ type: 'contact', nanny });
        navigateTo(Screen.Login);
        return;
    }
    if (currentUser.subscription?.status === 'active') {
      setContactNannyInfo(nanny);
    } else {
      setPendingAction({ type: 'contact', nanny });
  ...
  const handleAddNanny = async (nannyId: string) => {
    if (!currentUser) return;
    try {
        const updatedUser = await userService.addNanny(nannyId);
        setCurrentUser(updatedUser);
        alert(t('alert_nanny_added_dashboard'));
    } catch(e) { alert("Error adding nanny"); }
  };
  ...
  const handleRemoveNanny = async (nannyId: string) => {
    ...
    try {
        ...
        setCurrentUser(updatedUser);
    } catch(e) { alert("Error removing nanny"); }
  };

  const handleOpenRatingModal = (userToRate: User) => {
      setRatingTargetUser(userToRate);
  };

  const handleSubmitRating = async (targetUserId: string, ratingValue: number, comment: string) => {
      if (!currentUser) return;
      try {
  ...
  const handleOpenBookingModal = (nanny: User) => {
    if (!currentUser) {
        setPendingAction({ type: 'book', nanny });
        navigateTo(Screen.Login);
        return;
    }
    if (currentUser.subscription?.status !== 'active') {
        setPendingAction({ type: 'book', nanny });
        alert(t('alert_subscribe_to_book'));
        navigateTo(Screen.Subscription);
        return;
    }
    setBookingNannyInfo(nanny);
  };
  ...
  const handleOpenTaskModal = (nanny: User) => {
      setTaskModalNanny(nanny);
  };
  ...
  const handleDeleteMessage = async (contextId: string, messageId: string) => {
      ...
      const filterMsgs = (item: any) => ({
          ...item,
          messages: (item.messages || []).filter((m: ChatMessage) => m.id !== messageId)
      });

      if (activities.some(a => a.id === contextId)) {
          setActivities(prev => prev.map(a => a.id === contextId ? filterMsgs(a) : a));
      } else if (sharedOutings.some(o => o.id === contextId)) {
           setSharedOutings(prev => prev.map(o => o.id === contextId ? filterMsgs(o) : o));
      } else if (skillRequests.some(s => s.id === contextId)) {
           setSkillRequests(prev => prev.map(s => s.id === contextId ? filterMsgs(s) : s));
  ...
  const handleDeleteAllMessages = async (contextId: string) => {
       ...
       if (activities.some(a => a.id === contextId)) {
          setActivities(prev => prev.map(a => a.id === contextId ? clearMsgs(a) : a));
           ...
           setSharedOutings(prev => prev.map(o => o.id === contextId ? clearMsgs(o) : o));
      } else if (skillRequests.some(s => s.id === contextId)) {
  ...
  const handleCreateOuting = async (outingData: any) => {
    ...
    try {
        ...
        setSharedOutings(prev => [newOuting, ...prev]);
  ...
  const handleRequestOutingJoin = async (outing: SharedOuting, childName: string, childAge: number, emergencyContactName: string, emergencyContactPhone: string) => {
    ...
    try {
        ...
        setSharedOutings(prev => prev.map(o => o.id === outing.id ? updatedOuting : o));
  ...
  const handleUpdateOutingRequestStatus = async (outingId: string, parentId: string, status: 'accepted' | 'declined') => {
    try {
        ...
        setSharedOutings(prev => prev.map(o => o.id === outingId ? updatedOuting : o));
  ...
  const handleCreateSkillRequest = async (requestData: any) => {
      ...
      try {
          ...
          setSkillRequests(prev => [newSkill, ...prev]);
  ...
  const handleMakeSkillOffer = async (request: SkillRequest, offerAmount: number, message: string) => {
      ...
      try {
          ...
          setSkillRequests(prev => prev.map(r => r.id === request.id ? updatedSkill : r));
  ...
  const handleUpdateSkillOfferStatus = async (requestId: string, helperId: string, status: 'accepted' | 'declined') => {
      try {
          ...
          setSkillRequests(prev => prev.map(r => r.id === requestId ? updatedSkill : r));
  ...
  const currentUserAddedNannies = useMemo(() => {
    ...
    return (currentUser.addedNannyIds || [])
      .map(id => approvedNannies.find(n => n.id === id))
      .filter((n): n is User => !!n);
  }, [currentUser, approvedNannies]);

  const userBookingRequests = useMemo(() => {
    if (!currentUser) return [];
    if (currentUser.userType === 'parent') {
        return bookingRequests
            .filter(req => req.parentId === currentUser.id)
            .map(req => {
        ...
        return bookingRequests
            .filter(req => req.nannyId === currentUser.id)
            .map(req => ({...req, parent: { id: req.parentId, fullName: req.parentName, photo: '' } as User}));
    }
  }, [currentUser, bookingRequests, approvedNannies]);
  ...
  const renderScreen = () => {
    ...
    if (viewingNannyId && currentScreen === Screen.NannyProfileDetail) {
      ...
      const isAdded = currentUser?.addedNannyIds?.includes(viewingNannyId);
    ...
    switch (currentScreen) {
      case Screen.Welcome:
        return <WelcomeScreen onSelectUserType={handleSelectUserType} onLogin={() => navigateTo(Screen.Login)} />;
      case Screen.SignUp:
        return <SignUpScreen userType={userTypeForSignup} onSignUp={handleSignUp} onBack={goBack} onLogin={() => navigateTo(Screen.Login)} error={error} />;
      case Screen.Login:
        return <LoginScreen onLogin={handleLogin} onBack={goBack} onSignUp={() => navigateTo(Screen.SignUp)} onForgotPassword={() => navigateTo(Screen.ForgotPassword)} error={error} />;
      case Screen.ForgotPassword:
        return <ForgotPasswordScreen onBack={goBack} onSubmit={handleForgotPassword} />;
      case Screen.Questionnaire:
        return currentUser ? <Questionnaire user={currentUser} onSubmit={submitAssessment} error={error} onBack={goBack}/> : null;
      case Screen.Loading:
        return (
          <div className="flex flex-col items-center justify-center text-center p-8 min-h-[400px]">
            <LoadingSpinner />
            <h2 className="text-2xl font-semibold text-[var(--text-secondary)] mt-6">{t('loading_title')}</h2>
            <p className="text-[var(--text-light)] mt-2">{t('loading_message')}</p>
          </div>
        );
      case Screen.Result:
        return currentUser?.assessmentResult ? <ResultScreen result={currentUser.assessmentResult} onContinue={handleContinueFromResult} onRestart={() => navigateTo(Screen.Questionnaire, true)} onBack={goBack} isSuspended={!!(currentUser.suspendedUntil && new Date(currentUser.suspendedUntil) > new Date())} /> : null;
      case Screen.NannyProfileForm:
        return currentUser ? <NannyProfileForm user={currentUser} onSubmit={handleNannyProfileSubmit} onBack={goBack} /> : null;
      case Screen.ParentProfileForm:
        return currentUser ? <ParentProfileForm user={currentUser} onSubmit={handleParentProfileSubmit} onBack={goBack} /> : null;
      case Screen.Subscription:
        return currentUser ? <SubscriptionScreen onSubscribe={handleSubscribe} onBack={goBack} /> : null;
      ...
      case Screen.CommunityActivities:
        return currentUser ? <CommunityActivitiesScreen user={currentUser} activities={activities} onBack={goBack} onCreateActivity={() => setIsCreateActivityModalOpen(true)} onJoinActivity={handleJoinActivity} onOpenChat={(activity) => setActiveChat({ type: 'activity', item: activity })} /> : null;
      case Screen.ChildOutings:
        const enrichedOutings = sharedOutings.map(o => {
            return { ...o, isHostVerified: false };
        });
        return currentUser ? <ChildOutingScreen user={currentUser} outings={enrichedOutings} onBack={goBack} onCreateOuting={() => setIsCreateOutingModalOpen(true)} onRequestJoin={setRequestOutingInfo} onOpenChat={(outing) => setActiveChat({ type: 'outing', item: outing })} onRateHost={(hostId) => {
             const host = approvedNannies.find(n => n.id === hostId) || { id: hostId, fullName: 'Host', photo: '' } as User; 
             setRatingTargetUser(host);
        }} /> : null;
      case Screen.SkillMarketplace:
        return currentUser ? <SkillMarketplaceScreen user={currentUser} requests={skillRequests} onBack={goBack} onCreateRequest={() => setIsCreateSkillRequestModalOpen(true)} onMakeOffer={setMakeOfferSkillRequestInfo} onUpdateOffer={handleUpdateSkillOfferStatus} onOpenChat={(skill) => setActiveChat({ type: 'skill', item: skill })} /> : null;
      default:
        return <WelcomeScreen onSelectUserType={handleSelectUserType} onLogin={() => navigateTo(Screen.Login)} />;
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center">
      ...
      {requestOutingInfo && (
          <RequestOutingJoinModal 
            outing={requestOutingInfo} 
            onClose={() => setRequestOutingInfo(null)} 
            onSubmit={handleRequestOutingJoin} 
            existingRequests={requestOutingInfo.requests}
      ...
      {activeChat && currentUser && (
          <ChatModal 
            activity={activeChat.type === 'activity' ? activeChat.item as Activity : undefined}
            outing={activeChat.type === 'outing' ? activeChat.item as SharedOuting : undefined}
            skillRequest={activeChat.type === 'skill' ? activeChat.item as SkillRequest : undefined}
            bookingRequest={activeChat.type === 'booking' ? activeChat.item as BookingRequest : undefined}
      ...
      <Header 
        ...
        onEditProfile={
            (currentUser?.userType === 'parent' || (currentUser?.userType === 'nanny' && currentUser?.assessmentResult?.decision === 'Approved')) 
            ? handleEditProfile 
            : undefined
        }
      ...
      {showAiAssistant && currentUser && <AiAssistant user={currentUser} currentScreen={currentScreen} />}

      <footer className="text-center p-4 text-[var(--text-accent)] text-sm">
        <p>
          {t('footer_text')}{' '}
          <span className="font-bold text-xs animate-rainbow">
            Moubarak
          </span>
