    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-sm mx-auto">
        ...
        <div>
          ...
          <input type="text" id="cardNumber" value={cardNumber} onChange={e => validateAndSet('cardNumber', e.target.value, /^[0-9\s]*$/, t('error_numbers_only'))} placeholder={t('placeholder_card_number')} required className={inputStyles} />
          {errors.cardNumber && <p className="text-red-500 text-xs mt-1">{errors.cardNumber}</p>}
        </div>
        <div className="flex gap-4">
          <div className="flex-1">
            <label htmlFor="expiryDate" className={labelStyles}>{t('payment_expiry_date')}</label>
            <input type="text" id="expiryDate" value={expiryDate} onChange={e => validateAndSet('expiryDate', e.target.value, /^[0-9/\s]*$/, t('error_numbers_only'))} placeholder={t('placeholder_expiry')} required className={inputStyles} />
            {errors.expiryDate && <p className="text-red-500 text-xs mt-1">{errors.expiryDate}</p>}
          </div>
          <div className="flex-1">
            <label htmlFor="cvv" className={labelStyles}>{t('payment_cvv')}</label>
            <input type="text" id="cvv" value={cvv} onChange={e => validateAndSet('cvv', e.target.value, /^[0-9]*$/, t('error_numbers_only'))} placeholder={t('placeholder_cvv')} required className={inputStyles} />
